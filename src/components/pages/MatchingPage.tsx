...
export function MatchingPage({ page, answer, onAnswer }: MatchingPageProps) {
  ...
  const [matches, setMatches] = useState<Record<number, number>>(answer?.matches || {});
    ...
    <div className="space-y-4">
      <Card>
        <CardContent className="p-6">
          <p className="text-sm text-gray-600 mb-4">Match the items by clicking on the left, then the right:</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              {pairs.map((pair: any, index: number) => {
                ...
                const correct = showResult && isCorrectMatch(index, matchedRight);
                ...
                return (
                  <Button
                    ...
                    } ${showResult ? (correct ? 'border-green-500 bg-green-50' : 'border-red-500 bg-red-50') : ''}`}
            ...
            <div className="space-y-2">
              {rightItems.map((item: string, index: number) => {
                ...
                const correct = showResult && isCorrectMatch(Number(matchedLeftIndex), index);
                ...
                return (
                  <Button
                    ...
                    variant="outline"
                    className={`w-full justify-start h-auto py-3 ${
                      isMatched && !isChecked ? 'border-violet-500 bg-violet-50' : ''
                    } ${showResult ? (correct ? 'border-green-500 bg-green-50' : 'border-red-500 bg-red-50') : ''}`}
          ...
          {isChecked && (
            <div className="mt-4">
              {Object.entries(matches).every(([left, right]) => isCorrectMatch(Number(left), right as number)) ? (
